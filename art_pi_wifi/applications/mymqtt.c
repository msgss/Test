...
//主题
static char tx_topic[20];//发布
static char rx_topic[20];//订阅
...
//    mqtt_set_password (client, "abpassword");
    ...
    mqtt_set_keep_alive_interval(client, 40);
    ...
    mqtt_subscribe(client,"led002",QOS0,mqtt_subscribe_topic_handle ); //订阅
    diconnet_times=0;
    while(1)
    {
       mqtt_sleep_ms(5000);  
       ...
       //mqtt连接管理
       if(client->mqtt_client_state!=CLIENT_STATE_CONNECTED)//mqtt tcp断开
       {
